...
const visualAssets = {
  boundarysupport: {
    ...
    caption:
      "BoundarySupport changes context while learning only from pixel-preserved neighboring patches. Figure from my preprint.",
    altZh: "BoundarySupport 框架图，展示改变后的上下文、像素保持不变的邻近补丁、固定预算记忆与重建目标",
    captionZh: "BoundarySupport 改变周边语境，但只学习像素保持不变的邻近补丁。图源：本人的预印本。",
  },
  cleancon: {
    ...
    caption:
      "CLEANCON separates candidate eligibility from the fixed memory builder to test why the cleanest memory is not always best. Figure from my preprint.",
    altZh: "CLEANCON 原理图，将候选图像资格与固定的全局记忆构建器分离",
    captionZh: "CLEANCON 将候选资格与固定记忆构建器分离，以检验为何最干净的记忆并不总是最佳。图源：本人的预印本。",
  },
  procon: {
    ...
    caption: "ProCon replaces a single nearest anchor with local projection. Figure from my official repository.",
    altZh: "ProCon 方法图，对比硬最近邻检索与软局部投影",
    captionZh: "ProCon 以局部投影取代单一最近邻锚点。图源：本人的官方代码仓库。",
  },
  gcr: {
    ...
    caption: "GCR separates cross-head routing from within-head anomaly scoring. Figure from my preprint.",
    altZh: "GCR 流程图，包含冻结的 OpenCLIP 特征、类别原型库、几何一致路由与异常评分",
    captionZh: "GCR 将跨检测头路由与头内异常评分分离。图源：本人的预印本。",
  },
  "memory-sam": {
    ...
    caption: "Memory-SAM converts retrieved visual correspondence into automatic SAM2 prompts. Figure from my official repository.",
    altZh: "Memory-SAM 架构图，展示样例检索、对比点选择与 SAM2 舌体分割",
    captionZh: "Memory-SAM 将检索得到的视觉对应关系转化为自动 SAM2 提示。图源：本人的官方代码仓库。",
  },
} as const;
