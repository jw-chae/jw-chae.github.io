  ...
  stages: [
    {
      ...
      formula: "F_\\ell(x) = f_\\ell(x) \\in \\mathbb{R}^{P\\times C}, \\qquad P = 28\\cdot 28,\\quad C = 768,\\quad \\ell \\in \\{-3,-6,-8,-9\\}",
    ...
    },
    {
      ...
      formula: "a_i = \\operatorname{TopMean}_{0.5\\%}\\!\\big(\\operatorname{median}_b\\, r_{i,p,b}\\big), \\qquad T = \\{\\, i : a_i \\le \\operatorname{median}_j a_j \\,\\}",
    ...
    },
    {
      ...
      formula: "C = P(A)\\cup P(D), \\qquad R = \\operatorname{Dilate}\\big(P(A),\\,2\\big)\\setminus C",
    ...
    },
    {
      ...
      formula: "\\mu_t = \\arg\\max_{q}\\ \\min_{s<t}\\lVert q-\\mu_s\\rVert^2, \\qquad A_K = \\frac{p_{\\mathrm{mem}}}{p_{\\mathrm{pool}}}",
    ...
    },
    {
      ...
      formula: "r_c(x) = \\frac{1}{M_r}\\sum_{p\\in P_r}\\min_k \\lVert q_p - \\mu_{c,k}\\rVert^2, \\qquad \\hat c = \\arg\\min_c r_c(x)",
    ...
    },
    {
      ...
      formula: "w_j \\propto \\exp\\!\\big(-\\lVert z-m_j\\rVert^2/\\tau\\big), \\quad \\hat z = \\sum_j w_j m_j, \\quad r = \\lVert z-\\hat z\\rVert\n(k=1 \\;\\Rightarrow\\; r = s_{\\mathrm{NN}})",
    ...
    },
    {
      ...
      formula: "S_\\ell(p) = \\operatorname{median}_b R_{\\ell,b}(p), \\qquad S_{\\mathrm{map}}(p) = \\frac{1}{|L|}\\sum_\\ell S_\\ell(p)",
    ...
    },
    {
      ...
      formula: "\\phi(S) = \\big[\\sigma_S,\\ \\operatorname{TopMean}_r(S),\\ \\mathrm{TV}(S)\\big], \\quad z = (\\phi-\\mu)/\\sigma\nD_{\\mathrm{struct}} = \\lVert z\\rVert_\\infty, \\qquad S_{\\mathrm{hyb}} = S_{\\mathrm{base}} + \\lambda\\, D_{\\mathrm{struct}}",
...
export const copyZh: ShowcaseCopy = {
  ...
  stages: [
    { id: "tokens", num: "01", kicker: "固定表征", title: "一张图像变成 784 个 token", body: "冻结的 DINOv2 ViT-B/14 把 392 × 392 图像编码为 28 × 28 的 768 维补丁 token，并取多个深度。没有任何训练；下游所有工作都是在决定正常 token 如何被收集、保留、检索与比较。", formula: "F_\\ell(x) = f_\\ell(x) \\in \\mathbb{R}^{P\\times C}, \\qquad P = 28\\cdot 28,\\quad C = 768,\\quad \\ell \\in \\{-3,-6,-8,-9\\}", note: "上方的开场动画即此步骤：像素 → 补丁网格 → 特征空间中的 token。红色 token 是与真值裂纹重叠的单元。", paper: "ProCon" },
    { id: "eligibility", num: "02", kicker: "候选证据 · 资格", title: "哪些训练图像有资格参与", body: "在存储任何东西之前，候选池本身就是一个决策。CLEANCON 用二十个袋外支持库的软投影残差，衡量其他训练图像解释某张图像的程度，只把分数较低的一半交给不变的记忆构建器。检测器、构建器与最终记忆容量都不改变。", formula: "a_i = \\operatorname{TopMean}_{0.5\\%}\\!\\big(\\operatorname{median}_b\\, r_{i,p,b}\\big), \\qquad T = \\{\\, i : a_i \\le \\operatorname{median}_j a_j \\,\\}", note: "三张污染图像排到最前，中位数门控将其剔除。提高保留率会有一张重新进入，但论文中 P-AP 在最干净点之后仍持续上升：纯度本身并不能为有用的记忆排序。", paper: "CLEANCON" },
    { id: "support", num: "03", kicker: "候选证据 · 缺失的支持", title: "干净图像遗漏了缺陷旁的正常补丁", body: "即使候选池完全干净，也可能不完整。缺陷图像的大多数补丁仍是正常的，但紧邻缺陷的正常补丁处在任何干净图像都无法提供的语境中，因而没有参照。BoundarySupport 有意制造这种语境：向干净图像插入合成缺陷，排除插入与混合触及的全部 token，只保留像素保持不变的环带作为新的正常证据。", formula: "C = P(A)\\cup P(D), \\qquad R = \\operatorname{Dilate}\\big(P(A),\\,2\\big)\\setminus C", note: "插入区 A、混合像素 D、排除集 C，以及保留环 R。在 MVTec 固定记忆预算下，真实缺陷周围同样的环带恢复了 94.7% 的先知增益。", paper: "BoundarySupport" },
    { id: "memory", num: "04", kicker: "保留", title: "只保留少量正常 token 作为参照", body: "存储全部候选 token 太慢，PatchCore 类流程保留一个核心集：贪心最远优先选择，用 K 个中心覆盖特征空间。覆盖偏爱稀有点。若少量异常图像越过了门控，其稀有 token 恰恰是选择器最先抓取的对象。", formula: "\\mu_t = \\arg\\max_{q}\\ \\min_{s<t}\\lVert q-\\mu_s\\rVert^2, \\qquad A_K = \\frac{p_{\\mathrm{mem}}}{p_{\\mathrm{pool}}}", note: "调节预算并切换选择器。随机选择使污染保持在池内比例（A_K ≈ 1）；全局最远优先则把污染成倍拉入记忆，正如 CLEANCON 在 1% 预算下测得的 16 至 40 倍。", paper: "CLEANCON" },
    { id: "routing", num: "05", kicker: "路由", title: "按几何选择检测头，而不是按异常分数", body: "类别依次到来且测试图像无标签时，检测器必须先选择类别头，然后才能比较任何东西。跨头比较异常分数并不稳定：尺度不同，缺陷也可能翻转选择。GCR 用少量采样补丁的平均最近原型距离路由，然后只在该头内评分。", formula: "r_c(x) = \\frac{1}{M_r}\\sum_{p\\in P_r}\\min_k \\lVert q_p - \\mu_{c,k}\\rVert^2, \\qquad \\hat c = \\arg\\min_c r_c(x)", note: "从一张属于头 2 且含四个缺陷补丁的图像中采样 32 个补丁。均值准则路由正确；对峰值敏感的 top-q 准则被拖向相邻的宽泛检测头。", paper: "GCR" },
    { id: "projection", num: "06", kicker: "比较与信任", title: "一个邻近锚点并不足够", body: "最近邻评分只问是否存在一个邻近的正常锚点。有缺陷的补丁可能恰好落在某个孤立锚点旁而被判为正常，即假正常匹配。ProCon 改为询问补丁能否由其局部正常邻域重建：按距离加权 k 个锚点，并对投影残差评分。", formula: "w_j \\propto \\exp\\!\\big(-\\lVert z-m_j\\rVert^2/\\tau\\big), \\quad \\hat z = \\sum_j w_j m_j, \\quad r = \\lVert z-\\hat z\\rVert\n(k=1 \\;\\Rightarrow\\; r = s_{\\mathrm{NN}})", note: "移动鼠标即可移动测试 token。按下预设按钮把它停在孤立锚点旁：虚线的硬最近邻距离仍然很小，而残差骤增。把 k 设为 1，两条规则重合。", paper: "ProCon" },
    { id: "consensus", num: "07", kicker: "共识", title: "信任峰值之前先跨记忆达成一致", body: "单个核心集取决于一次特定的锚点选择，不走运的选择会制造虚假峰值。ProCon 为每个深度构建 B 个随机种子扰动的记忆库，对残差取中位数，再对各深度对齐后的残差图取均值。融合发生在每个库与每一层都产出同一量之后。", formula: "S_\\ell(p) = \\operatorname{median}_b R_{\\ell,b}(p), \\qquad S_{\\mathrm{map}}(p) = \\frac{1}{|L|}\\sum_\\ell S_\\ell(p)", note: "库 2 含有一个假峰。min 能去掉它却也削弱真实缺陷；mean 让它泄漏；median 去掉它的同时保留了共同的缺陷。", paper: "ProCon" },
    { id: "map", num: "08", kicker: "决策", title: "取最大值会丢掉信息", body: "图像级判断通常取异常图的最大值。最大值相同的两张图可以结构完全不同：正常图像上孤立的虚假峰值，与缺陷图像上分散、重尾的证据。StructCore 用三个数描述每张图，在正常图上标准化，并报告最强的校准偏差。", formula: "\\phi(S) = \\big[\\sigma_S,\\ \\operatorname{TopMean}_r(S),\\ \\mathrm{TV}(S)\\big], \\quad z = (\\phi-\\mu)/\\sigma\nD_{\\mathrm{struct}} = \\lVert z\\rVert_\\infty, \\qquad S_{\\mathrm{hyb}} = S_{\\mathrm{base}} + \\lambda\\, D_{\\mathrm{struct}}", note: "两张图被缩放到相同最大值，因此最大池化无法区分。增大缺陷范围，观察结构分数在最大值不变的情况下拉开差距。", paper: "StructCore" },
  ],
