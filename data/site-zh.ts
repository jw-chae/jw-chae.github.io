...
export const selectedResearchZh: ResearchItem[] = [
  {
    ...
    algorithm: "我先排除任何 RGB 像素发生变化的 token 区域，只保留合成缺陷周围两个 token 范围内像素不变的环带，并将其改变上下文后的特征作为正常证据。",
    result: "在固定记忆容量下，我们发现缺陷邻近正常补丁将 MVTec P-AP 从 73.34 提升至 76.95；BoundarySupport 在六种评估设置中带来 1.76 至 5.01 个百分点的提升。",
    status: "预印本，2026",
  ...
  },
  {
    ...
    algorithm: "我利用 20 个袋外支持库的软投影残差为图像评分，以中位数进行稳健聚合，再仅将较低风险的一半图像交给保持不变的记忆构建器。",
    result: "我们发现全局覆盖会将稀疏污染放大 16.04 至 40.61 倍；在 12 个匹配比较中，CLEANCON 均提升了宏平均 P-AP，但最干净的记忆并非性能最佳。",
    status: "预印本，2026",
  ...
  },
  {
    ...
    result: "我们在 HIT-Tongue 与包含 2,155 张智能手机图像的 SM-Tongue 上分别取得 0.984 和 0.973 mIoU，并公开 2,155 对去标识化的 512 x 512 图像与掩码用于可复现实验。",
...
const publicationTranslations: Array<Pick<Publication, "venue" | "contribution"> & { authorNote?: string }> = [
  ...
  },
  {
    ...
    contribution: "以前景与背景对比图生成自动 SAM2 提示，并公开 2,155 对图像与掩码。",
  },
  {
