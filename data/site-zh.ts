...
export const selectedResearchZh: ResearchItem[] = [
  {
    ...
    algorithm: "我排除与名义插入区域或任何检测到的 RGB 变化相交的 token，仅保留两个 token 范围内像素不变的环带，并将其改变上下文后的特征作为正常证据。",
    result: "在固定记忆容量下，缺陷邻近正常补丁将 MVTec P-AP 从 73.34 提升至 76.95，其中两个 token 范围内的邻近区域恢复了 94.70% 的增益。经过三个配对随机种子，BoundarySupport 在全部六个记忆与重建设置中均提升 P-AP。",
    status: "ICLR 审稿中",
  ...
  },
  {
    ...
    algorithm: "我根据 20 个袋外支持库计算逐补丁软投影残差，以残差最高的 0.5% 补丁为图像评分依据，再对支持库取中位数、对特征深度取均值，并将风险较低的一半图像交给保持不变的记忆构建器。",
    result: "在 1% 记忆预算下，全局最远优先选择将稀疏污染放大 16.04 至 40.61 倍。CLEANCON 将最终记忆污染率降至近零，并在全部 12 个匹配比较中提升类别宏平均 P-AP；但最干净的记忆并非性能最高。",
    status: "ICLR 审稿中",
  ...
  },
  {
    ...
    method: "我提出 ProCon，一种免训练、无需解码器的重建框架，以正常记忆上的软投影取代硬最近邻检索。",
    algorithm: "我将每个测试补丁软投影到局部正常邻域，对随机种子扰动的多组记忆残差取中位数，再对不同特征深度的残差图取均值。",
    result: "在冻结骨干网络条件下，我们在 MVTec AD、VisA 和 Real-IAD 上分别取得 99.8%、99.2% 和 93.2% 的图像级 AUROC。",
    status: "审稿中",
  ...
  },
  {
    ...
    algorithm: "我在共享冻结空间中，以 32 个采样补丁到各类别原型库的平均最近距离完成路由，再仅在选中的检测头内计算 LogSumExp 异常能量。",
    result: "在 MVTec AD 与 VisA 上，我们验证了 GCR 无需端到端表征学习即可显著提升路由稳定性，并将遗忘保持在接近零的水平。",
    status: "审稿中",
  ...
  },
  {
    ...
    topic: "我研究如何利用小规模标注记忆，在受控环境与非受控舌图像中自动生成可靠的 SAM2 提示，全程无需人工点击或参数更新。",
    method: "我用冻结的 DINOv3 检索候选样例，以专家掩码划分其前景与背景特征，再根据查询图像上的前景背景可分性完成重排序。",
    algorithm: "我计算 DINOv3 前景与背景相似度之差 S(i) = s_fg(i) - s_bg(i)，选出前三个前景对比点，并将其作为 SAM2 提示。",
    result: "我们在 HIT-Tongue 与包含 2,155 张智能手机图像的 SM-Tongue 上分别取得 0.984 和 0.973 mIoU，并公开 2,155 对去标识化的 512 x 512 图像与掩码。此前版本在 ICASSP 2026 与 MICCAI 2026 的平均审稿评分分别为 24/28 和 4.0/5.0。",
    status: "AAAI 2027 审稿中",
...
const publicationTranslations: Array<Pick<Publication, "venue" | "contribution"> & { authorNote?: string }> = [
  {
    venue: "ICLR 审稿中",
    contribution: "BoundarySupport 在固定预算下揭示缺陷邻近正常证据；两个 token 范围内的区域恢复了 94.70% 的增益。",
  },
  {
    venue: "ICLR 审稿中",
    contribution: "CLEANCON 通过袋外候选图像门控区分记忆纯度与记忆效用，并在 12 个匹配比较中提升宏平均 P-AP。",
  },
  {
    venue: "审稿中",
